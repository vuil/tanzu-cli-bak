  ...
  "completionType": 0
	...
	for _, spec := range tests {
		...
		t.Run(spec.test, func(t *testing.T) {
			...
			err = setupFakePlugin(dir, spec.plugin, spec.version, spec.cmdGroup, completionType, spec.postInstallResult, spec.args, spec.expected)
			assert.Nil(err)
...
func setupFakePlugin(dir, pluginName, version string, commandGroup plugin.CmdGroup, completionType uint8, postInstallResult uint8, args []string, expected string) error {
...
`, pluginName, pluginName, commandGroup, completionType, postInstallResult, pluginName, pluginName)
